-- Summary of data was completed by a visual inspection of the first 100 rows and subsequent queries
-- The dataset did not come with any documentation, which makes it difficult to justify certain cleaning decisions, such as dropping NULL columns, deduplicating matching records, and assigning arbitrary primary keys when no sole column was suitable; though I understand we can use composite keys.
-- Since the context of the data cannot be established and this is an assignment, I chose to demonstrate my competence in performing these cleaning techiques irrespective of their impact.
